...
class DBOps():
    def __init__(self, db_service_name="web_postgres-master"):
        self.db_container = safe_system_call(
            'docker ps --format "{{.Names}}" --filter "name={}"'.format(db_service_name)
        )
        self.db_name = self.exec_in_container("cat /run/secrets/postgres-user")

    def rollback_db(self, dump_name):
        self.exec_in_container(
            'psql -U {db_name} {db_name} -c "drop owned by {db_name} cascade;"'.format(db_name=self.db_name)
        )
        self.exec_in_container(
            "psql -U {db_name} {db_name} < /var/lib/postgres/data/{dump_name}".format(
    ...
    def exec_in_container(self, command, container=None):
        ...
        return safe_system_call(
            '"docker exec -t {container} /bin/bash -c "{command}"'.format(container=container, command=command)
        )

    def cleanup(self, dump_name):
        shutil.move(
            "/var/lib/docker/volumes/web_postgres_data_master/_data/{}".format(dump_name),
    ...
    def migrate_db(self):
        ...
        self.exec_in_container(
            "pg_dump -O -U \$(cat /run/secrets/postgres-user) \$(cat /run/secrets/postgres-user) > /var/lib/postgres/data/{}".format(dump_name)
        )
        dashboard_container = safe_system_call(
            'docker ps --format "{{.Names}}" --filter "name=web_dashboard"'
        )
        try:
